...
class ExtraDiskSize(pulumi.CustomResource):
    ...
    def __init__(__self__,
        ...
        *Preferable method to use.
        ...
        > **Warning:** Do not use this resource together with a `disk_auto_resize` alarm (see
        `Alarm`) on the same instance. Both control the instance's additional disk size, so the
        alarm can grow the disk out of band from Terraform. This resource does not refresh `extra_disk_size`
        ...
        `disk_auto_resize` alarm, not both.
        ...
        nodes = cloudamqp.get_nodes_output(instance_id=instance.id.apply(lambda x: int(x)))
        ```
